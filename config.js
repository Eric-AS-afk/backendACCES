export const FRONTEND_URL = process.env.FRONTEND_URL || "https://sienacces.site";

// Production MySQL connection defaults for the hosting environment
export const DB_HOST = process.env.DB_HOST || "sienacces.site";
export const DB_PORT = process.env.DB_PORT || 3306;
export const DB_DATABASE = process.env.DB_DATABASE || "qmbmwsjkey_cgp_SIENA";
export const DB_USER = process.env.DB_USER || "qmbmwsjkey_admin111";
export const DB_PASSWORD = process.env.DB_PASSWORD || "Eric30David01";

export const PORT = process.env.PORT || 3000;
