...
export const createRetiro = async (retiro) => {
  const { RET_CODIGO, US_USUARIO, PRO_PROVEEDOR, RET_FECHA, RET_SUBTOTAL, RET_RETENCION, RET_TOTAL, TIP_TIPO, RET_EVIDENCIA } = retiro;
  const [result] = await pool.query(
    'INSERT INTO sie_retiro (RET_CODIGO, US_USUARIO, PRO_PROVEEDOR, RET_FECHA, RET_SUBTOTAL, RET_RETENCION, RET_TOTAL, TIP_TIPO, RET_EVIDENCIA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [RET_CODIGO, US_USUARIO, PRO_PROVEEDOR, RET_FECHA, RET_SUBTOTAL, RET_RETENCION, RET_TOTAL, TIP_TIPO, RET_EVIDENCIA || null]
  );
...
export const updateRetiro = async (id, retiro) => {
  const { RET_CODIGO, US_USUARIO, PRO_PROVEEDOR, RET_FECHA, RET_SUBTOTAL, RET_RETENCION, RET_TOTAL, TIP_TIPO, RET_EVIDENCIA } = retiro;
  await pool.query(
    'UPDATE sie_retiro SET RET_CODIGO = ?, US_USUARIO = ?, PRO_PROVEEDOR = ?, RET_FECHA = ?, RET_SUBTOTAL = ?, RET_RETENCION = ?, RET_TOTAL = ?, TIP_TIPO = ?, RET_EVIDENCIA = ? WHERE RET_RETIRO = ?',
    [RET_CODIGO, US_USUARIO, PRO_PROVEEDOR, RET_FECHA, RET_SUBTOTAL, RET_RETENCION, RET_TOTAL, TIP_TIPO, RET_EVIDENCIA || null, id]
  );
  return id;
};
